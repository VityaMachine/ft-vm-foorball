const teamStatsCreator = ({
  played,
  win,
  draw,
  lose,
  goals,
}: IMatchesStandingsData) => ({
  played,
  win,
  draw,
  lose,
...
export const byTeamsFixturesParser = (data: IFixtureData[]) => {
  ...
  const teamsData = teamsIds.map((teamId) => {
    const homeTeamFixture = fixtures.find(
...
export const teamsResultsFromFixtures = (
  ...
  const fixturesAndResultsData = selectedData.map((teamData) => ({
    ...
    results: {
      ...
      draw: teamData.fixtures.filter((fixture) => fixture.result === "D")
        .length,
      lose: teamData.fixtures.filter((fixture) => fixture.result === "L")
        .length,
  ...
  const correctedRsultsData = fixturesAndResultsData.map((teamInfo) => {
    const isForCorrection = corrections.find(
      (item) =>
        item.teamId === teamInfo.teamId && item.leagueId === teamInfo.leagueId
    );
