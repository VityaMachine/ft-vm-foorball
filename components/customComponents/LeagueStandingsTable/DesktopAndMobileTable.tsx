...
export default function DesktopAndMobileTable({
  ...
  const [hoveredOpponentTeam, setHoveredOpponentTeam] = useState<null | number>(
    null
  );
  ...
  const visibleData = leagueData
    ? sortTableDataHandler(leagueData, sortField, sortDirection)
    : [];


  const placeTooltipTextHandler = (
                    ...
                    <Box sx={styles.infoTableHeadRowContainer(row.textAlign)}>
                      <Box sx={styles.infoTableHeadRowTextBox}>
                        ...
                        {row.isSortable && (
                          <IconButton
                            size="small"
                            sx={styles.tableHeadRowSortIconBtn(
                              sortField,
                              row.fieldName
                            )}
                            ...
                            {sortField === row.fieldName ? (
                              (sortDirection === "asc" && (
                                <KeyboardArrowDownIcon
                                  sx={styles.tableHeadRowSortIcon}
                                />
                              )) ||
                              (sortDirection === "desc" && (
                                <KeyboardArrowUpIcon
                  ...
                  <TableRow
                    sx={{
                      ...
                      bgcolor:
                        hoveredTeamRow &&
                        hoveredOpponentTeam &&
                        (hoveredOpponentTeam === team.teamId ||
                        hoveredTeamRow === team.teamId
                  ...
                  >
                    <TableCell width={69}>
                      {isTooltippedPlace(team, leagueParams) ? (
                        ...
                        >
                          <Box
                            sx={styles.infoTableBodyPlace(team, leagueParams)}
                          >
                            {team.leaguePosition}
                          </Box>
                        </Tooltip>
                      ) : (
                        <Box sx={styles.infoTableBodyPlace(team, leagueParams)}>
                          {team.leaguePosition}
                        </Box>
                      )}
                    </TableCell>
                    ...
                    <TableCell padding="none" width={185}>
                      <Link href={`/teams/${team.teamId}`}>
                        <Box sx={styles.infoTableBodyTeamNameCellBox}>
                          ...
                          <Typography
                            sx={styles.infoTableBodyTeamNameCellBoxText}
                          >
                            {team.teamNameData
                              ? language === "ua"
            ...
            <TableRow>
              {dataRows.map((row) => (
                <TableCell
                  key={row.id}
                  padding="none"
                  sx={styles.dataTableHeaderCell(row.isSortable)}
                >
                  <Tooltip
                    placement={row.tooltipPos}
                    title={
                  ...
                  >
                    <Box
                      sx={styles.dataTableHeaderCellContainer(
                        row.textAlign,
                        row.isSortable
                      )}
                    >
              ...
              (team) =>
                team && (
                  <TableRow
                    ...
                    key={team.teamId}
                    onMouseOver={() =>
                      setHoveredTeamRow(team.teamId ? team.teamId : null)
                    }
                    onMouseLeave={() => setHoveredTeamRow(null)}
                  >
                    <EnchancedTableDataCell
                      team={team}
                      field="games"
                      ...
                      <Box sx={styles.dataTableDataDesktopCellBox}>
                        {team.prev5.map((item) => (
                          <Tooltip
                            ...
                            title={
                              <Box
                                sx={styles.dataTableDataDesktopCellTooltipBox}
                              >
                                <Typography variant="caption">
                                  {item.date
                                    .toISOString()
                          ...
                          >
                            <Link
                              onMouseOver={() =>
                                setHoveredOpponentTeam(item.opponentId)
                              }
                              onMouseLeave={() => {
                            ...
                            >
                              <Box
                                ...
                                sx={{
                                  ...styles.dataTableLastMatchesDataBox(
                                    item.result
                                  ),
                          ...
                          >
                            <Link href={`/fixtures/${item.fixtureId}`}>
                              <Box
                                ...
                                sx={styles.dataTableNextMatchesImgBox(
                                  item.isHomeGame
                                )}
                              >
                                <Image
